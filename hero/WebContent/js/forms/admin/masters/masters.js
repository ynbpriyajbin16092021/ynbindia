...
function savestore()
{
	...
	if(totalValid < 0)
		...
		success:function(data,textStatus,jqXHR)
		{
			var responseType = getResponseType(data);
			var responseValue = getResponseValue(data);
			console.log("savestore2"+responseType)
			if(responseType == 'S')
				{
				displaySuccessMsg(data);
				clearStoreFields();
			...
			$.ajax({
				...
				success:function(res,textStatus,jqXHR)
				{
					...
					if(res[3] == "heroadminsettings"){	
						...
						setTimeout("location.href = 'userrole?applnid=2'",900);
						setTimeout("location.href = 'redirect_currency.html'",60000000000000000000);
					}
				}
			});
...
function getuserdetail(userid)
{
	$.ajax({
		...
		success:function(data,textStatus,jqXHR)
		{
			...
			$('#knownlanguagestext').val(userobj.knownlanguage);
...
function savenewuser()
{
	...
	isValidArray.push(validateonbuttonclick('#qualificationtext','input'));
	...
	isValidArray.push(validateonbuttonclick('#knownlanguagestext','input'));
	...
	userobj.qualification = $('#qualificationtext').val();

	userobj.dob = $('#dobtext').val();
	userobj.displayingname = $('#displayingnametext').val();
	userobj.knownlanguages = $('#knownlanguagestext').val();
