...
public class HerbzClinicUtil extends ClinicDAO{
	...
	private static Logger log = Logger.getLogger(HerbzClinicUtil.class);
	...
	public static ClinicResponse returnResponse(Map<String, Object> resultMap,ClinicResponse inventoryResponseOBJ)
	{
		if(resultMap != null)
		{
	...
	public static ClinicResponse returnExceptionResponse(ClinicResponse inventoryResponseOBJ,Exception e)
	{
	...
	public static <T> List<T> convertJSONArraytoList(String jsonArrayString,String className) throws JSONException, ClassNotFoundException
	{
		...
		for (int i = 0; i < jsonArr.length(); i++) {
			...
			T item = (T) HerbzClinicUtil.convertJSONtooOBJECT(jsonObj.toString(), className);
			list.add(item);
		}
		return list;
	}
...
public static int generateImageSeqforImageuplaod(JdbcTemplate jdbcTemplate,String uploadtype)
{
	...
	try
	{
		...
		{
			jdbcCALL = new SimpleJdbcCall(jdbcTemplate).withProcedureName("P_CLINIC_DOCTOR_CREATION");
			...
			inParamMap.put("P_DOB", HerbzClinicUtil.convertToSQLDate("01/01/2000"));//6
		...
		else if(uploadtype != null && uploadtype.equalsIgnoreCase("PAT"))
		{
			...
			jdbcCALL = new SimpleJdbcCall(jdbcTemplate).withProcedureName("P_clinic_patient_CREATION");
			inParamMap = new HashMap<String, Object>();
			
			inParamMap.put("P_PATIENT_ID", 0);//0
			inParamMap.put("P_FIRSTNAME", "");//1
			inParamMap.put("P_LASTNAME", "");//2
			inParamMap.put("P_DOB", HerbzClinicUtil.convertToSQLDate("01/01/2000"));//3
			inParamMap.put("P_GENDER", "");//4
