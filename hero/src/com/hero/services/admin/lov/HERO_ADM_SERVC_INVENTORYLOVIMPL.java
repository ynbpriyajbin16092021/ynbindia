...
List<Object> stockList = jdbcTemplate.query(stockQuery, new RowMapper() {
		...
		List<Object> usermenuList = jdbcTemplate.query(userMenuQuery, new RowMapper() {
			...
			public Object mapRow(ResultSet rs, int index) throws SQLException {
				...
				if(rs.getInt("issubmodule") == 0)
				{
					...
					map.put("style", "style='background-color: #66aae6;font-weight: bolder;color:white'");
				}
				else
				{
	...
	public List<Object> getemailTemplateList(String emailtemplateQuery) {
		...
		List<Object> LOVList = jdbcTemplate.query(emailtemplateQuery, new RowMapper() {
			...
			public Object mapRow(ResultSet rs, int arg1) throws SQLException {
				  ...
				  map.put("msgcontent", rs.getString("email_temp_content"));
				  index++;
				  return map;
			}
		});
	...
	public Map<String, Object> getDashboardDetails(HttpServletRequest httpRequest) {
		...
		String receptCountQuery ="SELECT  COUNT(*) FROM `hero_user` a JOIN `hero_user_type` b  ON a.`role` = b.`usertype_id` WHERE b.`usertype_id`= '4' ";
		String stockmanagerCountQuery ="SELECT  COUNT(*) FROM `hero_user` a JOIN `hero_user_type` b  ON a.`role` = b.`usertype_id` WHERE b.`usertype_id`= '2' ";
		...
		String userRecentQuery = "SELECT `username`,`usertype_name` FROM `hero_user` a JOIN `hero_user_type` b  ON a.`role` = b.`usertype_id`" 
		...
		String receptRecentQuery=
                "SELECT `username`,`phone` FROM `hero_user` a JOIN `hero_user_type` b  ON a.`role` = b.`usertype_id`" +"WHERE b.`usertype_id`= '4' ORDER BY `userid` DESC LIMIT 5"; 
		...
		String stockmanagerRecentQuery="SELECT `username`,`phone` FROM `hero_user` a JOIN `hero_user_type` b  ON a.`role` = b.`usertype_id` WHERE b.`usertype_id`= '2' ORDER BY `userid` DESC LIMIT 5";

		String taxRecentQuery="SELECT `TAX_NAME`,`TAX_RATE` FROM `hero_admin_tax`"
										+"ORDER BY `TAX_ID` DESC LIMIT 5 ";	
		String currencyRecentQuery="SELECT `currency`,`CURR_SYMBOL` FROM `hero_admin_currency`"
		...
		List<Object> topSellingProductList = (List<Object>) HERO_ADM_SERVC_HOSURINVENTORYUTIL.executeQueryWithList(topSellingProductQuery, jdbcTemplate);
